...
func handleConn(c net.Conn) {
	...
	for input.Scan() {
		wg.Add(1)
		// We need to pass a pointer to the wait group, otherwise
		// it is passed by value and wg.Done() in the function won't
		// affect the wait group in this scope as it is a different
		// object
	...
	fmt.Println("waiting")
	wg.Wait()
	fmt.Println("done waiting, closing conn")
	conn := c.(*net.TCPConn)
	conn.CloseWrite()
}
