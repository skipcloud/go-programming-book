...
/*
	...
	program of Section 3.3 or the 3D surface computation of Section 3.2,
...
func main() {
	...
	output := make(chan imgPoints)

	img := image.NewRGBA(image.Rect(0, 0, width, height))
	var wg sync.WaitGroup
	for py := 0; py < height; py++ {
		wg.Add(1)
		// I could go further and have the inner loop
		// also generate goroutines, and use a buffered
		// channel so the channels have some runway but
