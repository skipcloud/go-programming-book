...
func readList(lex *lexer, v reflect.Value) {
	...
	case reflect.Struct: // ((name value) ...)
		for !endList(lex) {
			...
			ident := identToName(lex.text(), v)
			f := v.FieldByName(ident)
			if f == reflect.Zero(reflect.TypeOf(f)).Interface() {
				panic("found it")
			}
			lex.next()
			read(lex, v.FieldByName(ident))
			lex.consume(')')
		}
...
func identToName(ident string, v reflect.Value) string {
	for i := 0; i < v.NumField(); i++ {
		field := v.Type().Field(i)
		t := field.Tag.Get("sexpr")
		alias := strings.Split(t, ",")[0]
		if alias == ident {
			return field.Name
		}
	}
