...
func (d *Decoder) Token() (Token, error) {
	...
	switch d.lex.token {
	case scanner.Ident:
		return Symbol{Name: d.lex.text()}, nil

	case scanner.String:
		s, _ := strconv.Unquote(d.lex.text()) // ignoring errors
		return String{Text: s}, nil

	case scanner.Int:
		i, _ := strconv.Atoi(d.lex.text()) // ignoring errors
	...
	return nil, errors.New(fmt.Sprintf("unexpected token %q", d.lex.text()))
}
